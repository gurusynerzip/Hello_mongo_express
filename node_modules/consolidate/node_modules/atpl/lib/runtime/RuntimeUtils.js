...
function interpretNumber(number, radix) {
    ...
    if (radix === undefined) {
        if (number.substr(0, 2).toLowerCase() == '0x')
            return exports.interpretNumber(number.substr(2), 16);
        if (number.substr(0, 2).toLowerCase() == '0b')
            return exports.interpretNumber(number.substr(2), 2);
        if (number.substr(0, 1) == '0')
            return exports.interpretNumber(number.substr(1), 8);
        radix = 10;
    }
    if (radix == 10)
...
function trim(value, characters) {
    if (characters !== undefined) {
        var regExpQuoted = '[' + exports.quoteRegExp(characters) + ']';
...
function number_format(value, decimal, decimal_point, decimal_sep) {
    ...
    value = exports.ensureNumber(value);
...
function range(from, to, step) {
    ...
    if (exports.isString(from) || exports.isString(to)) {
        return exports.rangeString(String(from), String(to), step);
    }
    return exports.rangeNumbers(from, to, step);
}
exports.range = range;
...
function rangeString(from, to, step) {
    if (typeof step === "undefined") { step = 1; }
    return exports.rangeNumbers(String(from).charCodeAt(0), String(to).charCodeAt(0), step).map(function (value, index, array) {
        ...
        for (i = 0; i < tree_length; i++) {
            ...
            if (node_type === 'string') {
                ...
                if (match[2]) {
                    ...
                    for (k = 0; k < match[2].length; k++) {
                        if (!arg.hasOwnProperty(match[2][k])) {
                            throw (exports.sprintf('[sprintf] property "%s" does not exist', match[2][k]));
                        }
                        arg = arg[match[2][k]];
                    }
                } else if (match[1]) {
                ...
                if (/[^s]/.test(match[8]) && (get_type(arg) != 'number')) {
                    throw (exports.sprintf('[sprintf] expecting number but found %s', get_type(arg)));
                }
                switch (match[8]) {
                    case 'b':
                        arg = arg.toString(2);
...
function empty(value) {
    if (value === null || value === undefined || value === false)
        return true;
    if (exports.isArray(value) || exports.isString(value))
...
function json_encode_circular(obj, already_encoded) {
    ...
    if (exports.isArray(obj)) {
        ...
        for (var n = 0; n < obj.length; n++) {
            if (n != 0)
                ret += ',';
            ret += exports.json_encode_circular(obj[n], already_encoded);
        }
        ret = '[' + ret + ']';
        ...
        for (var key in obj) {
            if (ret.length != 0)
                ret += ',';
            ret += JSON.stringify(key) + ':' + exports.json_encode_circular(obj[key], already_encoded);
        }
        ret = '{' + ret + '}';
    } else {
...
function getISO8601Week(aDate) {
    ...
    var d = new Date(exports.getISO8601Year(aDate), 0, 4);
...
function date(format, date, timezone) {
    ...
    return format.replace(/[dDjlNSwzWFmMntLoYyaABgGhHisu]/g, function (option) {
        switch (option) {
            ...
            case "S":
                return exports.getOrdinalFor(me.getDate());
            case "w":
                return me.getDay();
            case "z":
                return Math.ceil((jan1st - me) / 86400000);
            case "W":
                return ("0" + exports.getISO8601Week(me)).replace(/^.(..)$/, "$1");
            case "F":
            ...
            case "o":
                return exports.getISO8601Year(me);
            case "Y":
...
function inArray(value, array) {
    ...
    if (exports.isString(value) && exports.isString(array)) {
        return array.indexOf(value) != -1;
    }
    return false;
}
...
function callFunctionWithNamedArguments($context, $function, $arguments, $namedArguments) {
    if ($namedArguments !== null && $namedArguments !== undefined) {
        var argumentNames = exports.getFunctionParameterNames($function);
        var namedPairs = {};
        var unnamedList = [];
